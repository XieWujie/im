...
interface MsgDao {
    ...
    @Query("select * from message where messageId in (select messageId from message where createAt in (select MAX(createAt) from message group by conversationId))  and messageType>10 order  by createAt desc")
